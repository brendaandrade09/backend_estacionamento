...
app.post("/veiculos", (req, res) => {
    ...
    //         msg: "Placa, modelo e cor são obrigatórios!!!"
    //     })
    // }
    if (![placa, modelo, cor].every(
        (campo) => typeof campo === "string" && campo.trim()
    )) {
        return res.status(400).json({ msg: "Placa, modelo e cor são obrigatórios!!!" })
    }

    const placaNormalizada = placa.trim().toUpperCase();

    if (VEICULOS.length >= CAPACIDADE) {
        return res.status(409).json({ msg: "Estacionamento lotado!" });
    }

    if (VEICULOS.some((v) => v.placa === placaNormalizada)) {
        return res.status(409).json({ msg: "Veículo já estacionado!" });
    }
...
//consulta veículo específico
app.get("/veiculos/:id", (req, res) => {
    const id = Number(req.params.id);
    const veiculo = VEICULOS.find((v) => v.id === id);
    if (!veiculo) return res.status(400).json({ msg: "Veículo não encontrado" });
...
app.post("/veiculos/:id/saida", (req, res) => {
    ...
    res.status(200).json({ msg: "Saída registrada!", registro });
});
